...
FPS    = 80
SCALE  = 9.0   # affects how fast-paced the game is, forces should be adjusted as well
...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        if np.random.rand() < 0.5:
            offset = np.random.uniform(1.2, 1.6)
        else:
            offset = np.random.uniform(3.8, 4.2)

        self.lander = self.world.CreateDynamicBody(
            position = (VIEWPORT_W/SCALE/offset, initial_y),
            angle=0.0,
            fixtures = fixtureDef(
        ...
        for i in [-1,+1]:
            leg = self.world.CreateDynamicBody(
                position = (VIEWPORT_W/SCALE/offset - i*LEG_AWAY/SCALE, initial_y),
                angle = (i*0.05),
                fixtures = fixtureDef(
    ...
    def step(self, action):
        ...
        if (self.helipad_x1 <= pos.x <= self.helipad_x2) and done:
            reward = np.clip(100 - 8 * abs(vel.y) - 50 * abs(self.lander.angle), 0, None)
        else:
            reward = -0.03
    ...
    def render(self, mode='human'):
        ...
        for obj in self.particles + self.drawlist:
            for f in obj.fixtures:
                ...
                if type(f.shape) is circleShape:
                    ...
                    self.viewer.draw_circle(f.shape.radius + 0.5, 20, color=obj.color1).add_attr(t)
                    self.viewer.draw_circle(f.shape.radius + 0.5, 20, color=obj.color2, filled=False, linewidth=2).add_attr(t)
                else:
        ...
        for x in [self.helipad_x1, self.helipad_x2]:
            ...
            self.viewer.draw_polyline( [(x, flagy1), (x, flagy2)], color=(1,1,1), linewidth=5 )
