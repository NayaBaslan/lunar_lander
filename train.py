...
def preprocessing(X_train, y_train, X_valid, y_valid, conf):
    ...
    X_train = [np.array(crop(Image.fromarray(img, "RGB"))) for img in X_train]
    X_valid = [np.array(crop(Image.fromarray(img, "RGB"))) for img in X_valid]

    X_train = np.array([rgb2gray(img).reshape(1, 100, 100) for img in X_train])
    X_valid = np.array([rgb2gray(img).reshape(1, 100, 100) for img in X_valid])
    ...
    while idx + n_skip + 1 < length:
        ...
        while len(X_tmp) < hist_len:
            ...
            if idx + n_skip + 1 < length:
                ...
                break
        if len(X_tmp) != 0:
            X_tmp = np.array(X_tmp).astype('float32').reshape(hist_len, 100, 100)
            X_seq.append(X_tmp)
            y_seq.append(y_tmp)
